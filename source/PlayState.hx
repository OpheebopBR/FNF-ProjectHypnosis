...
class PlayState extends MusicBeatState
{
	...
	//Competitive Ratings
	//you can modify these values as you want, I recommend using the default ratings as some kind of reference for the values

	public static var osuRatings:Array<Dynamic> = [
		['F', 0.69], //0-68%
		['E', 0.75], //70-74%
		['D', 0.8], // 75-79%
		['C', 0.85], // 80-85%
		...
		['A', 0.95], // 90-94%
		['S', 1], // 95-99%
		['S+', 1] // like on the default ratings above, the value on this one isn't used, since S+ is always "1"
	];
	...
	override public function create()
	{
		...
		if(ClientPrefs.infoTextBorder == 'None') botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.NONE, FlxColor.BLACK);
	...
	public function RecalculateRating() {
			...
			// Competitive Ratings
		{
			...
			else
			{
				...
				if(ratingPercent >= 1)
				{
					ratingName = osuRatings[osuRatings.length-1][0];
				}
				else
				{
					for (i in 0...osuRatings.length-1)
					{
						if(ClientPrefs.osuRatings)
						if(ratingPercent < osuRatings[i][1])
						{
							ratingName = osuRatings[i][0];
							break;
						}
					}
				}
